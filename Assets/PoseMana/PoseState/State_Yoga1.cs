...
public class State_Yoga1 : MonoBehaviour {

    public PoseManager _pose_manager;

    /****ポーズオブジェクトの取得****/
    // 開脚のオブジェクトを取得
    public Pose_yoga1 _pose_yoga1;
    ...
    void Start()
    {
        ...
        _pose_yoga1 = GameObject.Find("Pose_yoga1").GetComponent<Pose_yoga1>();
    ...
    {
        /***関節が2つ以上当てはまったとき表示***/
        if (_pose_yoga1.R_arm_flag == true)
        {
            _pose_upper = true;
            if (_pose_yoga1.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_yoga1.R_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_yoga1.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
        ...
        if (_pose_yoga1.R_leg_flag == true)
        {
            _pose_lower = true;
            if (_pose_yoga1.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_yoga1.R_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_yoga1.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
    ...
    {
        if ((_pose_yoga1.R_arm_flag == true &&
            _pose_yoga1.L_arm_flag == true) ||
            (_pose_yoga1.R_leg_flag == true &&
            _pose_yoga1.L_leg_flag == true))
        {
            _pose_manager.Change_state(PoseManager.PoseState.Yoga_1);
        }
    ...
    public void ChanceDisplayTrue()
    {
        ...
        if (_pose_lower == true)
        {
            ...
            _Chance.transform.position = new Vector3(pos.x, pos.y, pos.z);
        }
        alpha = 1.0f;
    }
