...
public class State_Eiffelt : MonoBehaviour {
    ...
    /****ポーズオブジェクトの取得****/
    // エッフェル塔のオブジェクトを取得
    public Pose_eiffelt _pose_eiffelt;
    ...
    void Start () {
        ...
        _pose_eiffelt = GameObject.Find("Pose_Eiffelt").GetComponent<Pose_eiffelt>();
    ...
    {
        /***関節が2つ以上当てはまったとき表示***/
        if (_pose_eiffelt.R_arm_flag == true)
        {
            _pose_upper = true;
            if (_pose_eiffelt.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_eiffelt.R_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_eiffelt.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
        ...
        if (_pose_eiffelt.R_leg_flag == true)
        {
            _pose_lower = true;
            if (_pose_eiffelt.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_eiffelt.R_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_eiffelt.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
    ...
    {
        if ((_pose_eiffelt.R_arm_flag == true &&
            _pose_eiffelt.L_arm_flag == true) ||
            (_pose_eiffelt.R_leg_flag == true &&
            _pose_eiffelt.L_leg_flag == true))
        {
            _pose_manager.Change_state(PoseManager.PoseState.Eiffel_Tower);
        }
