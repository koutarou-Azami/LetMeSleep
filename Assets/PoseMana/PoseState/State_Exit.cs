...
public class State_Exit : MonoBehaviour {
    ...
    /****ポーズオブジェクトの取得****/
    // 出口のオブジェクトを取得
    public Pose_exit _pose_exit;
    public Image _rogo_exit;
    ...
    void Start () {
        ...
        _pose_exit = GameObject.Find("Pose_Exit").GetComponent<Pose_exit>();
        _rogo_exit = GameObject.Find("Rogo_EXIT").GetComponent<Image>();
        rogo_r = _rogo_exit.GetComponent<Image>().color.r;
        rogo_g = _rogo_exit.GetComponent<Image>().color.g;
        rogo_b = _rogo_exit.GetComponent<Image>().color.b;
        rogo_a = _rogo_exit.GetComponent<Image>().color.a;
    ...
    void Update()
    {
        ...
        _rogo_exit.GetComponent<Image>().color = new Color(rogo_r, rogo_g, rogo_b, rogo_a);
        CheckChance();
        FlagCheck();
        if (_rogo_flag == true)
        {
    ...
    {
        /***関節が2つ以上当てはまったとき表示***/
        if (_pose_exit.R_arm_flag == true)
        {
            _pose_upper = true;
            if (_pose_exit.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_exit.R_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_exit.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
        ...
        if (_pose_exit.R_leg_flag == true)
        {
            _pose_lower = true;
            if (_pose_exit.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_exit.R_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_exit.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
    ...
    {
        if ((_pose_exit.R_arm_flag == true &&
            _pose_exit.L_arm_flag == true) ||
            (_pose_exit.R_leg_flag == true &&
            _pose_exit.L_leg_flag == true))
        {
            _pose_manager.Change_state(PoseManager.PoseState.Exit);
        }
