...
public class State_Muaythai : MonoBehaviour {

    public PoseManager _pose_manager;

    /****ポーズオブジェクトの取得****/
    // 開脚のオブジェクトを取得
    public Pose_muaythai _pose_muaythai;
    ...
    void Start()
    {
        ...
        _pose_muaythai = GameObject.Find("Pose_muaythai").GetComponent<Pose_muaythai>();
    ...
    {
        /***関節が2つ以上当てはまったとき表示***/
        if (_pose_muaythai.R_arm_flag == true)
        {
            _pose_upper = true;
            if (_pose_muaythai.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_muaythai.R_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_muaythai.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
        ...
        if (_pose_muaythai.R_leg_flag == true)
        {
            _pose_lower = true;
            if (_pose_muaythai.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_muaythai.R_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_muaythai.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
    ...
    {
        if ((_pose_muaythai.R_arm_flag == true &&
            _pose_muaythai.L_arm_flag == true) ||
            (_pose_muaythai.R_leg_flag == true &&
            _pose_muaythai.L_leg_flag == true))
        {
            _pose_manager.Change_state(PoseManager.PoseState.Muay_thai);
        }
