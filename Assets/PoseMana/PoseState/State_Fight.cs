...
public class State_Fight : MonoBehaviour {

    public PoseManager _pose_manager;

    /****ポーズオブジェクトの取得****/
    // 開脚のオブジェクトを取得
    public Pose_Fight _pose_fight;
    ...
    void Start()
    {
        ...
        _pose_fight = GameObject.Find("Pose_Fight").GetComponent<Pose_Fight>();
    ...
    {
        /***関節が2つ以上当てはまったとき表示***/
        if (_pose_fight.R_arm_flag == true)
        {
            _pose_upper = true;
            if (_pose_fight.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_fight.R_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_fight.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
        ...
        if (_pose_fight.R_leg_flag == true)
        {
            _pose_lower = true;
            if (_pose_fight.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_fight.R_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_fight.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
    ...
    {
        if ((_pose_fight.R_arm_flag == true &&
            _pose_fight.L_arm_flag == true) ||
            (_pose_fight.R_leg_flag == true &&
            _pose_fight.L_leg_flag == true))
        {
            _pose_manager.Change_state(PoseManager.PoseState.Fight);
        }
