...
public class State_Hawk : MonoBehaviour {

    public PoseManager _pose_manager;

    /****ポーズオブジェクトの取得****/
    // 開脚のオブジェクトを取得
    public Pose_Hawk _pose_hawk;
    public Image _rogo_taka;
    ...
    void Start()
    {
        ...
        _pose_hawk = GameObject.Find("Pose_Hawk").GetComponent<Pose_Hawk>();
        _rogo_taka = GameObject.Find("Rogo_Taka").GetComponent<Image>();
        rogo_r = _rogo_taka.GetComponent<Image>().color.r;
        rogo_g = _rogo_taka.GetComponent<Image>().color.g;
        rogo_b = _rogo_taka.GetComponent<Image>().color.b;
        rogo_a = _rogo_taka.GetComponent<Image>().color.a;
    ...
    void Update()
    {
        ...
        _rogo_taka.GetComponent<Image>().color = new Color(rogo_r, rogo_g, rogo_b, rogo_a);
        CheckChance();
        FlagCheck();
        if (_rogo_flag == true)
        {
    ...
    {
        /***関節が2つ以上当てはまったとき表示***/
        if (_pose_hawk.R_arm_flag == true)
        {
            _pose_upper = true;
            if (_pose_hawk.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_hawk.R_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_hawk.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
        ...
        if (_pose_hawk.R_leg_flag == true)
        {
            _pose_lower = true;
            if (_pose_hawk.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_hawk.R_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_hawk.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
    ...
    {
        if ((_pose_hawk.R_arm_flag == true &&
            _pose_hawk.L_arm_flag == true) ||
            (_pose_hawk.R_leg_flag == true &&
            _pose_hawk.L_leg_flag == true))
        {
            _pose_manager.Change_state(PoseManager.PoseState.Pose_of_Turbulent_hawk);
        }
