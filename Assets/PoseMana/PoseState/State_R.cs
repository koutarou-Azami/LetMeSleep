...
public class State_R : MonoBehaviour {

    public PoseManager _pose_manager;

    /****ポーズオブジェクトの取得****/
    // 開脚のオブジェクトを取得
    public Pose_R _pose_r;
    ...
    void Start()
    {
        ...
        _pose_r = GameObject.Find("Pose_R").GetComponent<Pose_R>();
    ...
    {
        /***関節が2つ以上当てはまったとき表示***/
        if (_pose_r.R_arm_flag == true)
        {
            _pose_upper = true;
            if (_pose_r.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_r.R_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_r.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
        ...
        if (_pose_r.R_leg_flag == true)
        {
            _pose_lower = true;
            if (_pose_r.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_r.R_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_r.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
    ...
    {
        if ((_pose_r.R_arm_flag == true &&
            _pose_r.L_arm_flag == true) ||
            (_pose_r.R_leg_flag == true &&
            _pose_r.L_leg_flag == true))
        {
            _pose_manager.Change_state(PoseManager.PoseState.AlphaR);
        }
