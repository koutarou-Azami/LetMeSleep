...
public class State_Frog : MonoBehaviour {

    public PoseManager _pose_manager;

    /****ポーズオブジェクトの取得****/
    // 開脚のオブジェクトを取得
    public Pose_frog _pose_frog;
    ...
    void Start()
    {
        ...
        _pose_frog = GameObject.Find("Pose_frog").GetComponent<Pose_frog>();
    ...
    {
        /***関節が2つ以上当てはまったとき表示***/
        if (_pose_frog.R_arm_flag == true)
        {
            _pose_upper = true;
            if (_pose_frog.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_frog.R_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_frog.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
        ...
        if (_pose_frog.R_leg_flag == true)
        {
            _pose_lower = true;
            if (_pose_frog.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_frog.R_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_frog.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
    ...
    {
        if ((_pose_frog.R_arm_flag == true &&
            _pose_frog.L_arm_flag == true) ||
            (_pose_frog.R_leg_flag == true &&
            _pose_frog.L_leg_flag == true))
        {
            _pose_manager.Change_state(PoseManager.PoseState.Frog);
        }
