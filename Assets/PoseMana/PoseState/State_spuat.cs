...
public class State_spuat : MonoBehaviour {

    public PoseManager _pose_manager;

    /****ポーズオブジェクトの取得****/
    // 開脚のオブジェクトを取得
    public Pose_spuat _pose_spuat;
    ...
    void Start()
    {
        ...
        _pose_spuat = GameObject.Find("Pose_spuat").GetComponent<Pose_spuat>();
    ...
    {
        /***関節が2つ以上当てはまったとき表示***/
        if (_pose_spuat.R_arm_flag == true)
        {
            _pose_upper = true;
            if (_pose_spuat.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_spuat.R_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_spuat.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
        ...
        if (_pose_spuat.R_leg_flag == true)
        {
            _pose_lower = true;
            if (_pose_spuat.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_spuat.R_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_spuat.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
    ...
    {
        if ((_pose_spuat.R_arm_flag == true &&
            _pose_spuat.L_arm_flag == true) ||
            (_pose_spuat.R_leg_flag == true &&
            _pose_spuat.L_leg_flag == true))
        {
             _pose_manager.Change_state(PoseManager.PoseState.Spuat);
        }
