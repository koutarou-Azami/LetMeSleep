...
public class State_Yoga3 : MonoBehaviour {

    public PoseManager _pose_manager;

    /****ポーズオブジェクトの取得****/
    // 開脚のオブジェクトを取得
    public Pose_yoga3 _pose_yoga3;
    ...
    void Start()
    {
        ...
        _pose_yoga3 = GameObject.Find("Pose_yoga3").GetComponent<Pose_yoga3>();
    ...
    {
        /***関節が2つ以上当てはまったとき表示***/
        if (_pose_yoga3.R_arm_flag == true)
        {
            _pose_upper = true;
            if (_pose_yoga3.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_yoga3.R_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_yoga3.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
        ...
        if (_pose_yoga3.R_leg_flag == true)
        {
            _pose_lower = true;
            if (_pose_yoga3.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_yoga3.R_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_yoga3.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
    ...
    {
        if ((_pose_yoga3.R_arm_flag == true &&
            _pose_yoga3.L_arm_flag == true) ||
            (_pose_yoga3.R_leg_flag == true &&
            _pose_yoga3.L_leg_flag == true))
        {
            _pose_manager.Change_state(PoseManager.PoseState.Yoga_3);
        }
