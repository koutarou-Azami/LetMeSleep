...
public class State_Yoga2 : MonoBehaviour {

    public PoseManager _pose_manager;

    /****ポーズオブジェクトの取得****/
    // 開脚のオブジェクトを取得
    public Pose_yoga2 _pose_yoga2;
    ...
    void Start()
    {
        ...
        _pose_yoga2 = GameObject.Find("Pose_yoga2").GetComponent<Pose_yoga2>();
    ...
    {
        /***関節が2つ以上当てはまったとき表示***/
        if (_pose_yoga2.R_arm_flag == true)
        {
            _pose_upper = true;
            if (_pose_yoga2.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_yoga2.R_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_yoga2.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
        ...
        if (_pose_yoga2.R_leg_flag == true)
        {
            _pose_lower = true;
            if (_pose_yoga2.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_yoga2.R_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_yoga2.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
    ...
    {
        if ((_pose_yoga2.R_arm_flag == true &&
            _pose_yoga2.L_arm_flag == true) ||
            (_pose_yoga2.R_leg_flag == true &&
            _pose_yoga2.L_leg_flag == true))
        {
            _pose_manager.Change_state(PoseManager.PoseState.Yoga_2);
        }
