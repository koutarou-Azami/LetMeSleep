...
public class State_openLeg : MonoBehaviour {
    ...
    /****ポーズオブジェクトの取得****/
    // 開脚のオブジェクトを取得
    public Pose_opneLeg _pose_openleg;
    public Image _rogo_kaikyaku;
    ...
    void Update()
    {
        ...
        _rogo_kaikyaku.GetComponent<Image>().color = new Color(rogo_r, rogo_g, rogo_b, rogo_a);
        CheckChance();
        FlagCheck();
        if (_rogo_flag == true)
        {
    ...
    {
        /***関節が2つ以上当てはまったとき表示***/
        if (_pose_openleg.R_arm_flag == true)
        {
            _pose_upper = true;
            if (_pose_openleg.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_openleg.R_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_openleg.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
        ...
        if (_pose_openleg.R_leg_flag == true)
        {
            _pose_lower = true;
            if (_pose_openleg.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_openleg.R_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_openleg.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
    ...
    {
        if ((_pose_openleg.R_arm_flag == true &&
            _pose_openleg.L_arm_flag == true) ||
            (_pose_openleg.R_leg_flag == true &&
            _pose_openleg.L_leg_flag == true))
        {
            _pose_manager.Change_state(PoseManager.PoseState.Open_leg);
        }
