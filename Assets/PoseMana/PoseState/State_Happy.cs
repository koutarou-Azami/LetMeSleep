...
public class State_Happy : MonoBehaviour {
    ...
    void Start () {
        ...
        _pose_happy = GameObject.Find("Pose_Happy").GetComponent<Pose_Happy>();
    ...
    {
        /***関節が2つ以上当てはまったとき表示***/
        if (_pose_happy.R_arm_flag == true)
        {
            _pose_upper = true;
            if (_pose_happy.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_happy.R_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_happy.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
        ...
        if (_pose_happy.R_leg_flag == true)
        {
            _pose_lower = true;
            if (_pose_happy.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_happy.R_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_happy.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
    ...
    {
        if ((_pose_happy.R_arm_flag == true &&
            _pose_happy.L_arm_flag == true) ||
            (_pose_happy.R_leg_flag == true &&
            _pose_happy.L_leg_flag == true))
        {
            _pose_manager.Change_state(PoseManager.PoseState.Happy);
        }
