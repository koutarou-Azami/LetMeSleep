...
public class State_Zenkutu : MonoBehaviour {
    public PoseManager _pose_manager;

    /****ポーズオブジェクトの取得****/
    // 前屈のオブジェクトを取得
    public Pose_Zenkutu _pose_zenkutu;
    ...
    void Start () {
        ...
        _pose_zenkutu = GameObject.Find("Pose_Zenkutu").GetComponent<Pose_Zenkutu>();
    ...
    {
        /***関節が2つ以上当てはまったとき表示***/
        if (_pose_zenkutu.R_arm_flag == true)
        {
            _pose_upper = true;
            if (_pose_zenkutu.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_zenkutu.R_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_zenkutu.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
        ...
        if (_pose_zenkutu.R_leg_flag == true)
        {
            _pose_lower = true;
            if (_pose_zenkutu.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_zenkutu.R_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_zenkutu.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
    ...
    {
        if ((_pose_zenkutu.R_arm_flag == true &&
            _pose_zenkutu.L_arm_flag == true) ||
            (_pose_zenkutu.R_leg_flag == true &&
            _pose_zenkutu.L_leg_flag == true))
        {
            _pose_manager.Change_state(PoseManager.PoseState.Zenkutu);
        }
