...
public class State_X : MonoBehaviour {
    ...
    /****ポーズオブジェクトの取得****/
    // アルファベットXのオブジェクトを取得
    public Pose_X _pose_x;
    public Image _rogo_x;
    ...
    void Start () {
        ...
        _pose_x = GameObject.Find("Pose_X").GetComponent<Pose_X>();
        _rogo_x = GameObject.Find("Rogo_X").GetComponent<Image>();
        rogo_r = _rogo_x.GetComponent<Image>().color.r;
        rogo_g = _rogo_x.GetComponent<Image>().color.g;
        rogo_b = _rogo_x.GetComponent<Image>().color.b;
        rogo_a = _rogo_x.GetComponent<Image>().color.a;
    ...
    void Update()
    {
        ...
        _rogo_x.GetComponent<Image>().color = new Color(rogo_r, rogo_g, rogo_b, rogo_a);
        CheckChance();
        FlagCheck();
        if (_rogo_flag == true)
        {
    ...
    {
        /***関節が2つ以上当てはまったとき表示***/
        if (_pose_x.R_arm_flag == true)
        {
            _pose_upper = true;
            if (_pose_x.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_x.R_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_x.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
        ...
        if (_pose_x.R_leg_flag == true)
        {
            _pose_lower = true;
            if (_pose_x.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_x.R_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_x.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
    ...
    {
        if ((_pose_x.R_arm_flag == true &&
             _pose_x.L_arm_flag == true) ||
            (_pose_x.R_leg_flag == true &&
             _pose_x.L_leg_flag == true))
        {
            _pose_manager.Change_state(PoseManager.PoseState.AlphaX);
        }
