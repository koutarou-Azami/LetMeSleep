...
public class State_BodyBuli : MonoBehaviour {

    public PoseManager _pose_manager;
    /****ポーズオブジェクトの取得****/
    // ボディービルのオブジェクトを取得
    public Pose_BodyBuli _pose_bodybuli;
    ...
    void Start () {
        ...
        _pose_bodybuli = GameObject.Find("Pose_BodyBuli").GetComponent<Pose_BodyBuli>();
    ...
    {
        /***関節が2つ以上当てはまったとき表示***/
        if (_pose_bodybuli.R_arm_flag == true)
        {
            _pose_upper = true;
            if (_pose_bodybuli.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_bodybuli.R_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_bodybuli.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
        ...
        if (_pose_bodybuli.R_leg_flag == true)
        {
            _pose_lower = true;
            if (_pose_bodybuli.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_bodybuli.R_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_bodybuli.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
    ...
    {
        if ((_pose_bodybuli.R_arm_flag == true &&
            _pose_bodybuli.L_arm_flag == true) ||
            (_pose_bodybuli.R_leg_flag == true &&
            _pose_bodybuli.L_leg_flag == true))
        {
            _pose_manager.Change_state(PoseManager.PoseState.Bodybuilding);
        }
