...
public class State_Gymnastice : MonoBehaviour {

    public PoseManager _pose_manager;

    /****ポーズオブジェクトの取得****/
    // 開脚のオブジェクトを取得
    public Pose_Gymnastice _pose_gymnastice;
    ...
    void Start()
    {
        ...
        _pose_gymnastice = GameObject.Find("Pose_Gymnastice").GetComponent<Pose_Gymnastice>();
    ...
    {
        /***関節が2つ以上当てはまったとき表示***/
        if (_pose_gymnastice.R_arm_flag == true)
        {
            _pose_upper = true;
            if (_pose_gymnastice.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_gymnastice.R_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_gymnastice.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
        ...
        if (_pose_gymnastice.R_leg_flag == true)
        {
            _pose_lower = true;
            if (_pose_gymnastice.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_gymnastice.R_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_gymnastice.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
    ...
    {
        if ((_pose_gymnastice.R_arm_flag == true &&
            _pose_gymnastice.L_arm_flag == true) ||
            (_pose_gymnastice.R_leg_flag == true &&
            _pose_gymnastice.L_leg_flag == true))
        {
            _pose_manager.Change_state(PoseManager.PoseState.Gymnastics);
        }
