...
public class State_F : MonoBehaviour {
    public PoseManager _pose_manager;

    /****ポーズオブジェクトの取得****/
    // アルファベットFのオブジェクトを取得
    public Pose_F _pose_f;
    ...
    void Start () {
        ...
        _pose_f = GameObject.Find("Pose_F").GetComponent<Pose_F>();
    ...
    {
        /***関節が2つ以上当てはまったとき表示***/
        if (_pose_f.R_arm_flag == true)
        {
            _pose_upper = true;
            if (_pose_f.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_f.R_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_f.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
        ...
        if (_pose_f.R_leg_flag == true)
        {
            _pose_lower = true;
            if (_pose_f.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_f.R_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_f.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
    ...
    {
        if ((_pose_f.R_arm_flag == true &&
           _pose_f.L_arm_flag == true) ||
           (_pose_f.R_leg_flag == true &&
           _pose_f.L_leg_flag == true))
        {
            _pose_manager.Change_state(PoseManager.PoseState.AlphaF);
        }
