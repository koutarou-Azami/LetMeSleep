...
public class State_H : MonoBehaviour {
    ...
    /****ポーズオブジェクトの取得****/
    // アルファベットHのオブジェクトを取得
    public Pose_H _pose_h;
    public Image _rogo_h;
    ...
    void Start () {
        ...
        _pose_h = GameObject.Find("Pose_H").GetComponent<Pose_H>();
        _rogo_h = GameObject.Find("Rogo_H").GetComponent<Image>();
        rogo_r = _rogo_h.GetComponent<Image>().color.r;
        rogo_g = _rogo_h.GetComponent<Image>().color.g;
        rogo_b = _rogo_h.GetComponent<Image>().color.b;
        rogo_a = _rogo_h.GetComponent<Image>().color.a;
    ...
    void Update()
    {
        ...
        _rogo_h.GetComponent<Image>().color = new Color(rogo_r, rogo_g, rogo_b, rogo_a);
        CheckChance();
        FlagCheck();
        if (_rogo_flag == true)
        {
    ...
    {
        /***関節が2つ以上当てはまったとき表示***/
        if (_pose_h.R_arm_flag == true)
        {
            _pose_upper = true;
            if (_pose_h.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_h.R_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_h.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
        ...
        if (_pose_h.R_leg_flag == true)
        {
            _pose_lower = true;
            if (_pose_h.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_h.R_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_h.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
    ...
    {
        if ((_pose_h.R_arm_flag == true &&
            _pose_h.L_arm_flag == true) ||
            (_pose_h.R_leg_flag == true &&
            _pose_h.L_leg_flag == true))
        {
            _pose_manager.Change_state(PoseManager.PoseState.AlphaH);
        }
