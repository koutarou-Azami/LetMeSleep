...
public class State_Painfull1 : MonoBehaviour {

    public PoseManager _pose_manager;

    /****ポーズオブジェクトの取得****/
    // 開脚のオブジェクトを取得
    public Pose_painfullPose2 _pose_painfullpose2;
    ...
    void Start()
    {
        ...
        _pose_painfullpose2 = GameObject.Find("Pose_painfullpose2").GetComponent<Pose_painfullPose2>();
    ...
    {
        /***関節が2つ以上当てはまったとき表示***/
        if (_pose_painfullpose2.R_arm_flag == true)
        {
            _pose_upper = true;
            if (_pose_painfullpose2.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_painfullpose2.R_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_painfullpose2.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
        ...
        if (_pose_painfullpose2.R_leg_flag == true)
        {
            _pose_lower = true;
            if (_pose_painfullpose2.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_painfullpose2.R_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_painfullpose2.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
    ...
    {
        if ((_pose_painfullpose2.R_arm_flag == true &&
            _pose_painfullpose2.L_arm_flag == true) ||
            (_pose_painfullpose2.R_leg_flag == true &&
            _pose_painfullpose2.L_leg_flag == true))
        {
            _pose_manager.Change_state(PoseManager.PoseState.Painfull2);
        }
