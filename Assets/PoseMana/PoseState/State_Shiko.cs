...
public class State_Shiko : MonoBehaviour {
    ...
    /****ポーズオブジェクトの取得****/
    // 四股のオブジェクトを取得
    public Pose_shiko _pose_shiko;
    public Image _rogo_shiko;
    ...
    void Start () {
        ...
        _pose_shiko = GameObject.Find("Pose_Shiko").GetComponent<Pose_shiko>();
        _rogo_shiko = GameObject.Find("Rogo_Shiko").GetComponent<Image>();
        rogo_r = _rogo_shiko.GetComponent<Image>().color.r;
        rogo_g = _rogo_shiko.GetComponent<Image>().color.g;
        rogo_b = _rogo_shiko.GetComponent<Image>().color.b;
        rogo_a = _rogo_shiko.GetComponent<Image>().color.a;
    ...
    void Update()
    {
        ...
        _rogo_shiko.GetComponent<Image>().color = new Color(rogo_r, rogo_g, rogo_b, rogo_a);
        CheckChance();
        FlagCheck();
        if (_rogo_flag == true)
        {
    ...
    {
        /***関節が2つ以上当てはまったとき表示***/
        if (_pose_shiko.R_arm_flag == true)
        {
            _pose_upper = true;
            if (_pose_shiko.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_shiko.R_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_shiko.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
        ...
        if (_pose_shiko.R_leg_flag == true)
        {
            _pose_lower = true;
            if (_pose_shiko.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_shiko.R_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_shiko.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
    ...
    {
        if ((_pose_shiko.R_arm_flag == true &&
            _pose_shiko.L_arm_flag == true) ||
            (_pose_shiko.R_leg_flag == true &&
            _pose_shiko.L_leg_flag == true))
        {
            _pose_manager.Change_state(PoseManager.PoseState.Shico);
        }
