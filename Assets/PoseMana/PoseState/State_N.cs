...
public class State_N : MonoBehaviour {
    ...
    void Start()
    {
        ...
        _pose_n = GameObject.Find("Pose_N").GetComponent<Pose_N>();
    ...
    {
        /***関節が2つ以上当てはまったとき表示***/
        if (_pose_n.R_arm_flag == true)
        {
            _pose_upper = true;
            if (_pose_n.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_n.R_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_n.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
        ...
        if (_pose_n.R_leg_flag == true)
        {
            _pose_lower = true;
            if (_pose_n.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_n.R_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_n.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
    ...
    {
        if ((_pose_n.R_arm_flag == true &&
            _pose_n.L_arm_flag == true) ||
            (_pose_n.R_leg_flag == true &&
            _pose_n.L_leg_flag == true))
        {
            _pose_manager.Change_state(PoseManager.PoseState.AlphaN);
        }
