...
public class State_U : MonoBehaviour {
    ...
    {
        /***関節が2つ以上当てはまったとき表示***/
        if (_pose_u.R_arm_flag == true)
        {
            _pose_upper = true;
            if (_pose_u.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_u.R_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_u.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
        ...
        if (_pose_u.R_leg_flag == true)
        {
            _pose_lower = true;
            if (_pose_u.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_u.R_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_u.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
    ...
    {
        if ((_pose_u.R_arm_flag == true &&
            _pose_u.L_arm_flag == true) ||
            (_pose_u.R_leg_flag == true &&
            _pose_u.L_leg_flag == true))
        {
            _pose_manager.Change_state(PoseManager.PoseState.AlphaU);
        }
