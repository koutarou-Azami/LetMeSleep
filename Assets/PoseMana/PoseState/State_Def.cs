...
public class State_Def : MonoBehaviour {

    public PoseManager _pose_manager;

    /****ポーズオブジェクトの取得****/
    // 開脚のオブジェクトを取得
    public Pose_Defo _pose_defo;
    public Image _rogo_defo;
    ...
    void Update()
    {
        ...
        _rogo_defo.GetComponent<Image>().color = new Color(rogo_r, rogo_g, rogo_b, rogo_a);
        CheckChance();
        FlagCheck();
        if (_rogo_flag == true)
        {
    ...
    {
        /***関節が2つ以上当てはまったとき表示***/
        if (_pose_defo.R_arm_flag == true)
        {
            _pose_upper = true;
            if (_pose_defo.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_defo.R_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_defo.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
        ...
        if (_pose_defo.R_leg_flag == true)
        {
            _pose_lower = true;
            if (_pose_defo.L_leg_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_defo.R_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else if (_pose_defo.L_arm_flag == true)
            {
                ChanceDisplayTrue();
            }
            else
    ...
    {
        if ((_pose_defo.R_arm_flag == true &&
            _pose_defo.L_arm_flag == true) ||
            (_pose_defo.R_leg_flag == true &&
            _pose_defo.L_leg_flag == true))
        {
            _pose_manager.Change_state(PoseManager.PoseState.Deformed);
        }
